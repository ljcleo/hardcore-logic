...
'''
- task: Hanoi
- subtask: Custom target
- introduction: The target pillar is no longer the fixed last pillar, but a random one.
'''
class Hanoi_Gen02:
    def __init__(self, num_pegs=3, num_disks=3):
        ...
        self.target_peg = random.choice(self.pegs[1:])
    ...
    def format_start_and_goal(self):
        ...
        goal_state[self.target_peg] = list(range(self.num_disks, 0, -1))  # [N..1] 大到小
        goal_str = self.format_state(goal_state, f"Goal")

        return f"{start_str}\n{goal_str}"

if __name__ == "__main__":
    hanoi = Hanoi_Gen02(num_pegs=3, num_disks=5)
    print(f"initial_state(target peg: {hanoi.target_peg}):")
    print(hanoi.initial_state)
