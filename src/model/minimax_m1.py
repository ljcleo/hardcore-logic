...
class MinimaxM1Model(Model):
    ...
    async def query_full(
        ...
        *,
        ...
        think_reply: Reply = await client.chat(
            prompt,
            self.llm_model,
        ...
        if thought is not None and thought.startswith("<think>"):
            thought = thought.partition("<think>")[2]

        if stopped:
            response_reply: Reply = await self._query_comp_reply(
                client,
                prompt,
                None if thought is None else thought.strip("\n"),
                seed=seed,
    ...
    async def _query_comp_reply(
        ...
        *,
        ...
        return await client.complete(
            f"<begin_of_document><beginning_of_sentence>user name=user\n{prompt}<end_of_sentence>\n"
            f"<beginning_of_sentence>ai name=assistant\n<think>\n{thought}\n</think>\n\n",
            self.llm_model,
